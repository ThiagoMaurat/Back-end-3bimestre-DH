...
public class ContaService {
    ...
    private static final String SQL_UPDATE = "UPDATE conta SET saldoAtual = ?"
            + "WHERE id = ?";
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            atualizarConta.setDouble(1, 10.00 + mostrarSaldo(connection));
    ...
    public static void mostrarDadosDaConta(Connection connection) throws Exception {
        ...
        while (rs.next()) {
            ...
            + rs.getString(2) + " Conta: "
            + rs.getString(3) + " Saldo: "
            + rs.getDouble(4));
        }
    }

    public static double mostrarSaldo(Connection connection) throws Exception {
        String sqlSaldo = "SELECT saldoAtual FROM conta WHERE id = 1";
        Statement statement = connection.createStatement();
        ResultSet rs = statement.executeQuery(sqlSaldo);
        rs.next();
